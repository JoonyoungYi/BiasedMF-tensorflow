    ...
    with tf.Session() as sess:
        ...
        for epoch in range(N_EPOCHS):
            ...
            _, c, pred_batch = sess.run(
                ...
                feed_dict={
                    user_batch: train_data[:, 2],
                    movie_batch: train_data[:, 0],
                    rating_batch: train_data[:, 1]
                })
            ...
            print("E loss:" + str(
                round(
                    np.sqrt(
                        np.mean(np.power(pred_batch - train_data[:, 1], 2))),
                    2)))
        ...
        pred_batch = sess.run(
            [prediction],
            feed_dict={
                user_batch: test_data[:, 1],
                movie_batch: test_data[:, 0]
            })
        pred_batch = np.clip(pred_batch, 0, 5.0)
        print np.around(pred_batch * 2) / 2
...
movieId = np.array([
    10389, 11289, 470, 517, 8476, 6, 191, 312, 330, 481, 499, 524, 282, 577,
    1331
])
data[:, 1] = user_Id
data[:, 0] = movieId.astype(int)
