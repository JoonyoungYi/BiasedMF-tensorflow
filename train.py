...
def CollabFilterring(user_batch, movie_batch):
    ...
    w_user = tf.Variable(
        tf.random_normal([NUM_USERS, NUM_FEAT]) / np.sqrt(NUM_USERS))
    w_movie = tf.Variable(
        tf.random_normal([NUM_MOVIES, NUM_FEAT]) / np.sqrt(NUM_MOVIES))
    ...
    cost_reg = REG_PENALTY * tf.add(
        tf.nn.l2_loss(batch_w_movie), tf.nn.l2_loss(batch_w_user))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(N_EPOCHS):
            ...
            for i in range(num_batch_loop):
                ...
                errors.append(
                    np.mean(
                        np.power(pred_batch - train_data[i * BATCH_SIZE:(
                            i + 1) * BATCH_SIZE, 2], 2)))
            ...
            for i in range(num_batch_loop):
                ...
                errors.append(
                    np.mean(
                        np.power(pred_batch - test_data[i * TS_BATCH_SIZE:(
                            i + 1) * TS_BATCH_SIZE, 2], 2)))

            TS_epoch_loss = np.sqrt(np.mean(errors))
            RMSEts.append(TS_epoch_loss)
            ftime = time.time()
            remtime = (N_EPOCHS - epoch - 1) * (ftime - stime)
            print("Epoch" + str(epoch + 1) + " completed out of " + str(
                N_EPOCHS) + "; Train loss:" + str(round(TR_epoch_loss, 3)) +
                  "; Test loss:" + str(round(TS_epoch_loss, 3)))
            printTime(remtime)
